...
if (isset($_SESSION['role'])) {
    ...
    $query = "SELECT description, date_added, medpracNumber FROM medical_history_records WHERE IdNumber = ? ORDER BY date_added DESC LIMIT ?, ?";
    $stmt = mysqli_prepare($conn, $query);
    ...
    // fetch total of rows
    $records = $conn->query("SELECT * FROM medical_history_records WHERE IdNumber = $idNumber");
            ...
            <div class="col-auto col-md-3 col-xl-2 bg-transparent" style="padding: 0;">
                <div class="d-flex flex-column text-white" style="height: 85vh;">
                    <ul class="nav nav-pills flex-column mb-sm-auto mb-0 text-center" id="menu" style="background-color:#E4E4E4;">
                        ...
                        <li class="nav-item">
                            <a href="records-medical-records.php" class="nav-link py-5 fw-bold fs-4 text active">
                                <i class="fs-3 bi bi-clipboard2-pulse"></i><br><span class="d-none d-md-inline">MEDICAL RECORDS</span>
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="records-prescription.php" class="nav-link py-5 fw-bold fs-4 text">
            ...
            <div class="col position-relative">
                <div class="col-lg-12 text-center pt-5 px-lg-5">
                    <div class="appointments-box">
                        <p class="fs-1 fw-bold m-0 p-1">MEDICAL RECORDS</p>
                    </div>
                </div>
                            ...
                            <div class="col-lg-12 table-responsive">
                                ...
                                <table class="table table-bordered border-dark table-light lh-lg">
                                    <tbody class="col-sm-12">
                                        <!-- table header -->
                                        <tr>
                                            <td class="text-center fs-3 fw-bold px-lg-5" style="color: #000066;">Your Medical Record</td>
                                        </tr>
                                        <?php
                                        while ($row = mysqli_fetch_assoc($resultRecords)) {
                                            ...
                                            $medpracNumber = $row['medpracNumber'];

                                            // Fetch the full name of the medical practitioner
                                            $query = "SELECT fullName FROM medprac";
                                            $stmt = mysqli_prepare($conn, $query);
                                            // mysqli_stmt_bind_param($stmt, "s", $medpracNumber);
                                            mysqli_stmt_execute($stmt);
                                            $medpracResult = mysqli_stmt_get_result($stmt);
                                            $medpracRow = mysqli_fetch_assoc($medpracResult);
                                            $medpracFullName = $medpracRow['fullName'];
                                        ?>
                                            <tr>
                                                <td class="px-lg-5">
                                                    <!-- row head -->
                                            ...
                                            <tr>
                                                <td class="px-lg-5">
                                                    <!-- row head -->
                                                    <span class="fs-4 fw-bold" style="color: #000066;">Medical Details</span>
                                                    <br>
                                                    <!-- row details -->
                                                    <span class="fs-5">Consultation with: <?php echo $medpracFullName; ?></span><br>
                                                    <!-- <span class="fs-5">Description: <?php echo $description; ?></span><br> -->
