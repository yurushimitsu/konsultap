...
// Function to insert Medical history into the database
function insertMedicalHistory($conn, $idNumber, $medpracNumber, $description)
{
    $dateAdded = date('Y-m-d'); // Get current date
    $query = "INSERT INTO medical_history_records (IdNumber, medpracNumber, description, date_added) VALUES (?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "ssss", $idNumber, $medpracNumber, $description, $dateAdded);
    mysqli_stmt_execute($stmt);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['description']) && !empty($_POST['description'])) {
        ...
        insertMedicalHistory($conn, $selectedIdNumber, $idNumber, $description); // Pass $idNumber as medpracNumber
    }
}
?>
                        ...
                        <div class="col-12 border border-2 border-dark rounded shadow-bottom shadow" style="min-height: 50vh;">
                                ...
                                <div class="container-fluid">
                                    <div class="navbar-collapse justify-content-center" id="navbarNavAltMarkup">
                                        <div class="navbar nav-pills justify-content-center">
                                            <a class="nav-link px-lg-3 text-dark"  href="procedure-history.php?IdNumber=<?php echo urlencode($row['IdNumber']); ?>">Procedure History</a>
                                            <a class="nav-link px-lg-3 text-white" style="background-color: #000066;"href="medical-history.php?IdNumber=<?php echo urlencode($row['IdNumber']); ?>">Past Medical History</a>
                            ...
                            <!-- card information -->
                             <div class="row p-5" style="max-height: 300px; overflow-y: auto;">
                                <!-- Fetch Medical history from the database and display here -->
                                <?php
                                $medicalHistoryQuery = "SELECT title, description, date_added FROM medical_history_records WHERE IdNumber = ?";
                                $stmt = mysqli_prepare($conn, $medicalHistoryQuery);
                                mysqli_stmt_bind_param($stmt, "s", $selectedIdNumber);
                                mysqli_stmt_execute($stmt);
                                $medicalHistoryResult = mysqli_stmt_get_result($stmt);
                                // Check if there is record, else print no available record
                                if (mysqli_num_rows($medicalHistoryResult) > 0) {
                                    while ($medicalHistoryRow = mysqli_fetch_assoc($medicalHistoryResult)) {
                                        $title = $medicalHistoryRow['title'];
                                        $description = $medicalHistoryRow['description'];
                                        $date = date('F j, Y', strtotime($medicalHistoryRow['date_added']));
