...
// Function to insert Prescription history into the database
function insertPrescriptionHistory($conn, $idNumber, $description)
{
    $dateAdded = date('Y-m-d'); // Get current date
    $query = "INSERT INTO prescription_records (IdNumber, description, date_added) VALUES (?, ?, ?)";
    $stmt = mysqli_prepare($conn, $query);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['description']) && !empty($_POST['description'])) {
        ...
        insertPrescriptionHistory($conn, $selectedIdNumber, $description);
    }
}
?>
                    ...
                    <div class="col-lg-8 col-12 px-5 ">
                        <div class="col-12 border border-2 border-dark rounded shadow-bottom shadow" style="min-height: 50vh;">
                                ...
                                <div class="container-fluid">
                                    <div class="navbar-collapse justify-content-center" id="navbarNavAltMarkup">
                                        <div class="navbar nav-pills justify-content-center">
                                            ...
                                            <a class="nav-link px-lg-3 text-white"style="background-color: #000066;" href="prescription.php?IdNumber=<?php echo urlencode($row['IdNumber']); ?>">Prescription</a>
                            ...
                            <div class="row p-5" style="max-height: 300px; overflow-y: auto;">
                                ...
                                <?php
                                $prescriptionHistoryQuery = "SELECT title, description, date_added FROM prescription_records WHERE IdNumber = ?";
                                $stmt = mysqli_prepare($conn, $prescriptionHistoryQuery);
                                mysqli_stmt_bind_param($stmt, "s", $selectedIdNumber);
                                mysqli_stmt_execute($stmt);
                                $prescriptionHistoryResult = mysqli_stmt_get_result($stmt);
                                // Check if there is record, else print no available record
                                if (mysqli_num_rows($prescriptionHistoryResult) > 0) {
                                    while ($prescriptionHistoryRow = mysqli_fetch_assoc($prescriptionHistoryResult)) {
                                        $title = $prescriptionHistoryRow['title'];
                                        $description = $prescriptionHistoryRow['description'];
                                        $date = date('F j, Y', strtotime($prescriptionHistoryRow['date_added']));
