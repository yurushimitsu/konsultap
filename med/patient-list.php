...
if (isset($_SESSION['IdNumber'])) {
    ...
    $query = "SELECT fullName FROM medprac WHERE IdNumber = ?";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "s", $idNumber);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
...
$queries['students'] = "SELECT id, fullName, IdNumber, Email FROM students WHERE (disabled_at IS NULL OR disabled_at > CURDATE() - INTERVAL 4 YEAR)";
$queries['faculty'] = "SELECT id, fullName, IdNumber, Email FROM faculty WHERE (disabled_at IS NULL OR disabled_at > CURDATE() - INTERVAL 4 YEAR)";
...
//     // Append WHERE clause for search if search term is provided
//     if (!empty($search)) {
        ...
        <div class="row">
            ...
            <div class="col position-relative px-0 min-vh-100">
                <nav class="navbar navbar-expand-lg navbar-light bg-transparent border border-bottom">
                    <div class="container-fluid">
                        <a class="navbar-brand d-none d-md-inline" href="#">Patient List</a>
                ...
                <div class="col-md-12">
                        <h1 class="h1 pt-5 pl-5">Patient List</h1>
                                ...
                                <tbody>
                                    ...
                                    //         while ($row = mysqli_fetch_assoc($result)) {
                                            foreach ($limitResult as $row) {
                                                ...
                                                echo "<a href='procedure-history.php?IdNumber=" . urlencode($row['IdNumber']) . "' class='btn text-light' style='background-color: #000066;'><i class='bi bi-pencil-square'></i></a>";
                                                echo "</td>";
                                                echo "</tr>";
                                            }
                                    //         }
