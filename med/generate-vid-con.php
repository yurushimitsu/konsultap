...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    $videoconferenceId = mysqli_real_escape_string($conn, $_POST['videoconference_id']);

    // Query to check if the videoconference ID exists in the database
    $query = "SELECT * FROM videoconference WHERE videoconference_id = ?";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "s", $videoconferenceId);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);

    if (mysqli_num_rows($result) > 0) {
        // Redirect to videoconference.php with the videoconference_id
        header("Location: ../videoconference/videoconference.php?videoconference_id=" . urlencode($videoconferenceId));
        exit;
    } else {
        $error = "Invalid videoconference ID. Please try again.";
    }
}
...
<head>
    ...
    <title>KonsulTap︱Video Conference</title>
        ...
        <div class="row">
            ...
            <div class="col-sm-auto sticky-top shadow p-0" style="background-color: #006699; height:auto; max-height: 100vh; overflow-y: auto;">
                <div class="d-flex flex-sm-column flex-row flex-nowrap align-items-center sticky-top p-0" style="background-color: #006699;">
                    ...
                    <ul class="nav nav-pills nav-flush flex-sm-column flex-row flex-nowrap mb-auto mb-0 px-lg-3">
                        ...
                        <li class="nav-item">
                            <a href="generate-vid-con.php" class="nav-link nav-center py-3 fw-bold fs-5 text active">
            ...
            <div class="col position-relative px-0 min-vh-100">
                <nav class="navbar navbar-expand-lg navbar-light bg-transparent border border-bottom">
                    <div class="container-fluid">
                        <a class="navbar-brand d-none d-md-inline" href="#">Generate Video Consultation</a>
                    </div>
                </nav>
                <div class="container">
                    <div class="row">
                        <div class="col-12 p-lg-5">
                            <div class="col-12 border border-2 border-dark rounded shadow-bottom shadow">
                                ...
                                <div class="container mt-5">
                                    <h1 class="text-center">Enter Videoconference ID</h1>
                                    <div class="row justify-content-center">
                                        <div class="col-md-6 pb-5">
                                            ...
                                            <form action="" method="post" target="_blank">
                                                <div class="mb-3">
                                                    <label for="videoconference_id" class="form-label">Videoconference ID</label>
                                                    <input type="text" class="form-control" id="videoconference_id" name="videoconference_id" required>
                                                </div>
                                                <button type="submit" class="btn text-light" style="background-color: #000066;">Submit</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
...
<script>
        function clearForm() {
            document.getElementById('vidConForm').reset();
        }
    </script>