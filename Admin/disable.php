...
if (isset($_GET['IdNumber'])) {
    ...
    foreach ($tables as $table) {
        ...
        if (mysqli_num_rows($result) > 0) {
            // Update the status column to disable the user
            $updateQuery = "UPDATE $table SET status = 'disabled', disabled_at = CURDATE() WHERE IdNumber = ?";
            ...
            if (mysqli_stmt_affected_rows($updateStmt) > 0) {
                echo "<script>alert('User with ID $IdNumber has been disabled successfully.');</script>";
            } else {
                echo "<script>alert('Failed to disable the user. Please try again.');</script>";
            }
